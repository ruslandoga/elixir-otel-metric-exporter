defmodule OtelMetricExporter do
  ...
  defp log_metric_error(metric, {kind, reason, stacktrace}) do
    Logger.warning(
      "Failed to record telemetry metric #{inspect(metric.name)}: #{format_metric_error(kind, reason, stacktrace)}"
    )
  end

  defp format_metric_error(kind, reason, stacktrace) do
    Exception.format(kind, reason, stacktrace)
  rescue
    _exception -> "#{kind} with an unformattable reason"
  catch
    _kind, _reason -> "#{kind} with an unformattable reason"
  end
end
